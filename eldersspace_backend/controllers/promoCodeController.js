...
// ─── Upload codes to a campaign ──────────────────────────────────────────────
exports.uploadCodesToCampaign = async (req, res) => {
  let conn;
  const csvFilePath = req.file ? req.file.path : null;
  ...
  try {
    ...
    if (maxCodes !== null) {
      ...
      if (wouldExceed) {
        ...
        conn.release();
        return res.status(400).json({
          error: `เกินโควต้าแคมเปญ: มีอยู่แล้ว ${existing} โค้ด, อัพโหลดเพิ่มได้อีกสูงสุด ${maxCodes - existing} โค้ด (quota: ${maxCodes})`,
          quota_exceeded: true,
          current_count: existing,
          max_codes: maxCodes,
          slots_remaining: maxCodes - existing
        });
      }
    }
