...
function initTheme() {
    const savedTheme = localStorage.getItem('elderspace_theme') || 'dark';
...
function applyTheme(theme) {
    if (theme === 'light') {
        ...
        localStorage.setItem('elderspace_theme', 'light');
    } else {
        document.documentElement.classList.remove('light-mode');
        document.body.classList.remove('light-mode');
        localStorage.setItem('elderspace_theme', 'dark');
    }
    updateThemeIcon();
}
...
function updateThemeIcon() {
    ...
    const currentTheme = localStorage.getItem('elderspace_theme') || 'dark';
...
function getCompanyRowByIndex(index) {
    const rows = dashboardPayload?.sections?.companies;
    ...
    try {
        ...
        historyList.innerHTML = `
            ...
            ${history.map((item, idx) => `
                <div style="padding: 0.75rem; border-left: 2px solid var(--text-muted); margin-bottom: 0.75rem; padding-left: 1rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.25rem;">
                        <strong style="color: var(--text-primary);">
                            ${item.type === 'add' ? '<i class="fas fa-plus" style="color: var(--green);"></i>' : '<i class="fas fa-minus" style="color: var(--red);"></i>'} 
                            ${item.sourceType === 'admin' ? 'ผู้ดูแล' : item.sourceType}
                        </strong>
                        <span style="color: ${item.points > 0 ? 'var(--green)' : 'var(--red)'}; font-weight: bold;">
...
async function saveNewReward() {
    ...
    try {
        ...
        const category = document.getElementById('reward-category')?.value?.trim() || '';
...
function showRewardDetails() {
    ...
    if (!rewardSelect.value) {
        ...
        if (uploadHistorySection) uploadHistorySection.style.display = 'none';
        return;
    }

    if (uploadHistorySection) uploadHistorySection.style.display = 'block';
...
async function loadVerifierRewardCodes(rewardId) {
    ...
    try {
        ...
        // Use returned promo code rows directly
        const allCodes = codes.map(code => ({
            code: code.code,
            description: code.description,
            expiry_date: code.expiry_date,
            used_by_phone: code.used_by_phone || '-',
            status: code.status || (code.is_used ? 'used' : 'available'),
        ...
        const codesHtml = allCodes.map((code, idx) => {
            ...
            return `
                <div style="display: grid; grid-template-columns: 200px 1fr 150px 150px 150px auto; gap: 1rem; align-items: center; padding: 0.75rem; background: var(--bg-card); border-bottom: 1px solid var(--border); border-radius: 4px; margin-bottom: 0.5rem;">
                    ...
                    <div style="text-align: center; color: var(--text-secondary); font-size: 0.85rem;">${code.used_by_phone || '-'}</div>
        ...
        if (codesHtml) {
            const headerHtml = `
                <div style="display: grid; grid-template-columns: 200px 1fr 150px 150px 150px auto; gap: 1rem; padding: 0.75rem; background: var(--bg-card-alt); border-bottom: 2px solid var(--border); border-radius: 4px; margin-bottom: 0.5rem; font-weight: 600; color: var(--text-primary); font-size: 0.85rem;">
                    ...
                    <div style="text-align: center;">ถูกแลกไปโดยใคร</div>
